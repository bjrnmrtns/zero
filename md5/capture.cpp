	...
	void convertrgbtoI420(const unsigned char* data, vpx_image_t *image)
	{
		...
		for(size_t y = 0; y < image->h; y++)
		{
			for(size_t x = 0; x < image->w; x++)
			{
				...
				*(dest.pixw(x,image->h-y-1)) = 0.299 * r + 0.587 * g + 0.114 * b;
				*(dest.pixw(x,image->h-y-1)+1) = (-0.169 * r - 0.331 * g + 0.4998 * b) + 128;            
				*(dest.pixw(x,image->h-y-1)+2) = (0.4998 * r - 0.419 * g - 0.081 * b) + 128; 
			}
		}
		size_t i = 0;
		for(size_t y = 0; y < image->h; y++)
